...
description: "Use this agent when you need to design, implement, or extend the GEM Skills System - a contract-first, schema-validated framework for creating reusable, deterministic skill packs that wrap existing tools with execution plans, validation gates, and comprehensive documentation. Specifically invoke this agent when: (1) creating new skill specifications that need to align with tools.registry.json and repo constraints, (2) scaffolding complete skill packs with specs/runbooks/tests/prompts/changelogs, (3) implementing or modifying the skills engine (loader/validator/runner/planner/reporter), (4) designing JSON schemas for skill contracts and execution reports, (5) building the Skill Composer capability that generates skill skeletons from plain-English goals, (6) auditing existing skills for contract compliance or registry drift, (7) integrating new npm scripts for skill linting/indexing/verification/execution, or (8) producing deterministic execution reports with status outcomes (succeeded/failed/not_configured) that respect receipt doctrine and idempotency rules. This agent operates as a contract engineer, not a feature brainstormer - contract correctness always takes precedence over implementation speed.\\n\\n<example>\\nContext: User wants to add a new skill for validating lead data before CRM submission.\\nuser: \"I need to create a skill that validates lead objects against our CRM requirements and produces a standardized tool call plan\"\\nassistant: \"I'll use the Task tool to launch the gem-skills-architect agent to design and scaffold this lead validation skill pack with full contract compliance.\"\\n<commentary>\\nSince the user is requesting creation of a new skill within the GEM Skills System that requires schema alignment, tool registry validation, and deterministic execution planning, the gem-skills-architect agent must be invoked to ensure contract-first design.\\n</commentary>\\n</example>\\n\\n<example>\\nContext: User has modified tools.registry.json and needs to verify skill pack alignment.\\nuser: \"I just updated the quote.create tool schema - can you check if our existing skills need updates?\"\\nassistant: \"I'm going to use the Task tool to launch the gem-skills-architect agent to run a registry drift analysis and identify any skill packs that need contract updates.\"\\n<commentary>\\nSince tools.registry.json is authoritative law for the Skills System and changes to tool contracts may invalidate existing skill specifications, the gem-skills-architect agent must proactively audit and report misalignments.\\n</commentary>\\n</example>\\n\\n<example>\\nContext: User wants to implement a complex multi-tool workflow with validation gates.\\nuser: \"We need a skill that takes a quote acceptance, validates prerequisites, creates a job, assigns a tradesperson, and schedules the first visit - all with proper idempotency\"\\nassistant: \"I'll use the Task tool to launch the gem-skills-architect agent to design this multi-step workflow skill with DRY_RUN planning, validation gates, and idempotency considerations.\"\\n<commentary>\\nSince this requires orchestrating multiple tools with deterministic outcomes, validation checkpoints, and idempotency enforcement (owned by the worker, not bypassed by skills), the gem-skills-architect agent must create a contract-compliant skill specification.\\n</commentary>\\n</example>"
model: sonnet
color: pink
---
